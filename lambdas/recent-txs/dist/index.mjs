...
var FeeConflictError, InvalidSerializableTransactionError, TransactionNotFoundError, TransactionReceiptNotFoundError, TransactionReceiptRevertedError, WaitForTransactionReceiptTimeoutError;
...
// index.mjs
var RPC = process.env.RPC_URL || "http://host.docker.internal:8545";
var client = createPublicClient({ chain: hardhat, transport: http(RPC) });
var handler = async (event) => {
  try {
    ...
    const maxBlocks = Math.max(1, Math.min(50, Number(body?.count ?? 10)));
    ...
    const blocks = await Promise.all(
      blockNumbers.map(
        (n) => client.getBlock({ blockNumber: n, includeTransactions: true })
      )
    );
    const txs = blocks.flatMap(
      (b) => (b.transactions || []).map((tx) => ({
        hash: tx.hash,
        from: tx.from,
        to: tx.to,
